...
const TextInput = ({ onSubmit, onWaveToggle, isWaveMode, onBreathingToggle, isBreathingMode }: TextInputProps) => {
    ...
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-10 w-11/12 max-w-2xl px-4">
      <div className="glass-morphism rounded-2xl p-2 flex gap-2 shadow-2xl transition-all duration-300 hover:bg-white/15 hover:border-white/30">
        <button
          ...
          title={isBreathingMode ? 'Back to Sphere' : 'Breathing Orb'}
        >
          <Heart className={`w-5 h-5 ${isBreathingMode ? 'animate-pulse' : ''}`} />
        </button>
        <button
